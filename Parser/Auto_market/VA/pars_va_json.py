...
def get_original_parts(parts):
    ...
    for j, i in enumerate(original_parts):
        originals[j] = {'delivery_date': i.get('termDate'), 'price': i.get('price'), 'amount': i.get('amount'),
                        'seller': i.get("destination"), 'seller_rank': i.get('rating')}
    pprint(sorted(originals.items(), key=lambda x: x[1]['price']))


def get_analog(parts):
    analog = {}
    ...
    for part in analog_parts:
        ...
        offers = {'id': id_part, 'delivery_date': date_delivery, 'amount': amount, 'price': price,
                  'vendor': destination}
