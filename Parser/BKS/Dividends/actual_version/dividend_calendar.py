...
def write_json(obj_mame, file_name):
    ...
    и имя файла для сохранения данных(имя файла указывается без расширения файла). После создания файла
...
def get_calendar_json():
    """Функция отправляет запрос к api компании БКС для получения данных о дивидендных выплатах.
     На выходе функции, с помощью функции write_json, формируется json файл с актуальными данными по дивидендным
     выплатам(как данные о будущих дивидендах, так и ретроспектива по выплатам прошлых лет).
